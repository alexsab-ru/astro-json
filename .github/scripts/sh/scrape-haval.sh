...
export ID_XPATH="concat('haval-', replace(substring-before(substring-after((.//div[contains(@class, 'web_block_models__item-btn')]/a/@href)[1], '/models/'), '/'), '(/models/|haval-|new-|/$)', ''))"
export MODEL_XPATH="replace((.//div[contains(concat(' ', normalize-space(@class), ' '), ' web_block_models__item-title ')]/text()), 'НОВЫЙ ', '')"
